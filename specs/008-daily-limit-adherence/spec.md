...
1. **Given** a user already has a daily point limit, **When** they save a new daily point limit, **Then** the current day updates to use the new limit immediately.
...
**Independent Test**: Can be fully tested by reviewing Progress after a limit change and confirming the adherence count matches the unchanged past-day classifications plus the current effective limit for today.
...
2. **Given** a user changes the daily limit during the current day, **When** they open Progress afterward, **Then** the current day reflects the new limit and past completed days keep their prior adherence status.
...
- What happens when the user changes the daily point limit after already logging meals today? The current day must be recalculated against the new limit.
...
- **FR-003**: The system MUST apply an updated daily point limit to the current day immediately after the change is saved.
...
- **SC-004**: In acceptance testing, the progress adherence total matches the visible day-level history across at least one limit-change scenario.
...
- A limit change affects the whole current day once saved, even if some meals for that day were logged earlier under the prior limit.
