...
1. **Given** a user is adding a meal with a value of 4, **When** they set the count to 3 and save, **Then** the app records the meal as three identical meals and shows a total contribution of 12 wherever meal totals are shown.
...
3. **Given** a user is adding a meal, **When** they try to use a count below 1 or a non-whole number, **Then** the app prevents saving that count and guides the user back to a valid whole-number count.
...
2. **Given** a combined history row represents multiple exact meal entries, **When** the user edits the meal details or count, **Then** the row and totals update to match the edited grouped amount.
...
- Very large counts must be capped or rejected before saving if the count would make the meal history hard to review or cause totals that are clearly accidental.
...
- **FR-004**: Saving a meal with a count greater than 1 MUST make totals and summaries behave as if the user saved that exact meal separately that many times.
- **FR-005**: Meal history MUST combine exact meal entries from the same history day into one visible row.
- **FR-006**: A combined history row MUST show the count represented by that row when the count is greater than 1.
- **FR-007**: A combined history row MUST display the meal value multiplied by the count, while single entries continue to show their normal value.
- **FR-008**: Meal history MUST keep entries separate when any user-visible meal detail differs, including name, value, meal type, date, or notes.
- **FR-009**: Editing or removing a combined history row MUST update the represented meal count and all affected visible totals consistently.
- **FR-010**: Existing repeated exact meal entries MUST be combined in history without requiring users to recreate them.
- **FR-011**: Automated coverage MUST verify adding a counted meal, viewing combined history rows, and preserving separate rows for non-exact meals.

### Key Entities

- **Meal Entry**: A logged meal instance with user-visible details such as name, value, meal type, date, notes, and count.
...
- Very large accidental counts should be prevented with a reasonable product limit during planning.
